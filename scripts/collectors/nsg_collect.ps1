<#
Collect NSG security rules via Azure Resource Graph. Requires Az module and Az.ResourceGraph.
Writes evidence JSON and optionally posts to API.
#>
param(
    [string]$OutputPath = "./evidence_nsgs.json",
    [string]$ApiEndpoint = "http://localhost:8000/evidence"
)

if (-not (Get-Module -ListAvailable -Name Az.ResourceGraph)) {
    Write-Host "Az.ResourceGraph module not found. Run: Install-Module -Name Az.ResourceGraph -Scope CurrentUser" -ForegroundColor Yellow
}

try {
    # Ensure Az is connected
    if (-not (Get-AzContext -ErrorAction SilentlyContinue)) {
        Write-Host "Not connected to Az account. Run Connect-AzAccount" -ForegroundColor Yellow
    }

    $query = "Resources | where type =~ 'microsoft.network/networksecuritygroups' | extend securityRules = properties.securityRules | project id, name, location, subscriptionId, securityRules"
    ...
    Write-Error "Error collecting NSG evidence: $_"
}
